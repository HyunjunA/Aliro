...
describe("series of tests with mock file blob", () => {
  ...
  for (let row = 0; row < dataByFeature[0].length; row++) {
    ...
    for (let col = 0; col < featureNames.length; col++) {
      ...
      if (col < featureNames.length - 1) line += ",";
    }
    line += "\n";
    dataCSV += line;
  }
  ...
  function getFeatureTypesDefault() {
    return [
      instance.featureTypeCategorical,
      instance.featureTypeCategorical,
      instance.featureTypeNumeric,
      instance.featureTypeCategorical,
    ];
  }
  ...
  function errorModalIsShowing() {
    let res = {
      result:
        shallowDom.find("#error_modal_dialog").length > 0 &&
        shallowDom.find("#error_modal_dialog").at(0).prop("open"),
  ...
  it("DONE - test feature assignment behaviors via direct manipulation", () => {
    ...
    expect(instance.getFeatureType(featureNames[0])).toEqual(
      instance.featureTypeCategorical
    );
    ...
    expect(instance.getFeatureType(ordFeature)).toEqual(
      instance.featureTypeOrdinal
    );
    ...
    expect(instance.getCatFeatures().find((el) => el === ordFeature)).toEqual(
      undefined
    );
    ...
    expect(instance.validateFeatureName("The Spanish Inquisition")).toEqual(
      false
    );
    // Should return default type for invalid feature name
    expect(instance.getFeatureType("xkcd4eva")).toEqual(
      instance.featureTypeDefault
    );
    expect(instance.getFeatureDefaultType("Frank the Furter")).toEqual(
      instance.featureTypeDefault
    );
  });
  ...
  it("DONE - test dependent feature and prediction type behaviors via direct manipulation", () => {
    ...
    expect(instance.getFeatureType(dep1)).toEqual(
      instance.featureTypeDependent
    );
    ...
    expect(instance.getFeatureType(dep2)).toEqual(
      instance.featureTypeDependent
    );
    expect(instance.getDependentColumn()).toEqual(dep2);
    expect(instance.getFeatureType(dep1)).toEqual(
      instance.getFeatureDefaultType(dep1)
    );
    ...
    expect(instance.state.predictionType).toEqual(
      instance.defaultPredictionType
    );
  });
  ...
  it("test text-based categorical-type specification", () => {
    ...
    function updateCatText(text, cancel = false) {
      ...
      if (cancel)
        shallowDom
          .find("#cat_features_user_text_cancel_button")
          .at(0)
          .simulate("click"); //uses state vars
      else
        shallowDom
          .find("#cat_features_user_text_accept_button")
          .at(0)
          .simulate("click"); //uses state vars
      shallowDom.update();
    }
    ...
    updateCatText(
      featureNames.join(),
      true /*simulate clicking cancel button*/
    );
    expect(errorModalIsShowing().result).toBe(false);
    testFeatureTypes("default");
  ...
  it("test text-based ordinal-type specification", () => {
    ...
    function updateOrdText(text, cancel = false) {
      ...
      if (cancel)
        shallowDom
          .find("#ordinal_features_user_text_cancel_button")
          .at(0)
          .simulate("click"); //uses state vars to process text
      else
        shallowDom
          .find("#ordinal_features_user_text_accept_button")
          .at(0)
          .simulate("click"); //uses state vars to process text
      shallowDom.update();
    }
  ...
  it("test other UI elements", () => {
    ...
    let targetLabel = shallowDom.find(
      "#featureTypeTargetLabel-" + index.toString()
    );
    ...
    for (let row = 0; row < dataByFeature[0].length; row++) {
      featureNames.forEach((feature, index) => {
        ...
        expect(data.at(0).props().children.toString()).toEqual(
          dataByFeature[index][row].toString()
        );
      });
    }
  ...
  it("test generate file data", () => {
    ...
    instance.setFeatureType(
      featureNames[depColumnIndex],
      instance.featureTypeDependent
    );
