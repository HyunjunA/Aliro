...
//
// The enzyme wrapper method 'state' (and related methods) is not working on render(<my component>).
// I think it's because the component is wrapped with redux connect().
// So you can use render(<my component>).dive().state(),
//  or more direclty render(<my component>).dive().instance().state
...
describe('basic testing of FileUploadOrigin react component', () => {
  ...
  beforeEach(() => {
    fullDom = render(<FileUploadOrigin store={store} testProp='hello' />);
    //NOTE with react-redux, need 'dive()' to get the 'true' (not sure of the term) component instance.
    shallowDom = render(<FileUploadOrigin store={store}/>).dive();
  ...
  it('DONE - FileUploadOrigin component, test for existence', () => {
    ...
    expect(fullDom.testProp).toEqual('hello');
    expect(fullDom.name).toEqual('bar');
  })

  //snapshot of component
  //I believe shallow is better since it won't include child components
  it('DONE - full component snapshot', () => {
    expect(render(<FileUploadOrigin store={store}/>).dive()).toMatchSnapshot();
  })
...
describe('series of tests with mock file blob', () => {
  ...
  function errorModalIsShowing() {
    let res = {
      result: shallowDom.find("#error_modal_dialog").length > 0 && shallowDom.find("#error_modal_dialog").at(0).'open'),
  ...
  function closeErrorModal() {
    ...
    //shallowDomfireEvent.'keydown', {keyCoode: 27});
    //shallowDomfireEvent.'click');
    //shallowDomfireEvent.'click'); //tries to find the close icon on the modal, but it fails
  ...
  beforeAll(() => {
    fullDom = render(<FileUploadOrigin store={store} />);
    shallowDom = render(<FileUploadOrigin store={store}/>).dive();
    instance = shallowDom.instance();
  })
  afterAll(() => {
  ...
  it('test text-based categorical-type specification', () => {
    ...
    function updateCatText(text, cancel = false) {
      // Open the dialog
      const openButton = screen.getByTestId("cat_features_text_input_open_button");
      fireEvent.click(openButton);
    
      // Get the text input element
      const textInput = screen.getByTestId("categorical_features_text_area_input");
  ...
  it('test text-based ordinal-type specification', () => {
    ...
    function updateOrdText(text, cancel = false) {
      // Open the dialog
      const openButton = screen.getByTestId("ord_features_text_input_open_button");
      fireEvent.click(openButton);
    
      // Get the text input element
      const textInput = screen.getByTestId("ordinal_features_text_area_input");
    ...
    let value = ordTextInput.at(index).value;
  ...
  it('test other UI elements', () => {
    ...
    featureNames.forEach( (feature, index) => {
      ...
      expect(label.at(0).children[1]).toEqual(featureNames[index]);
      ...
      expect(drop.at(0).value).toEqual(getFeatureTypesDefault()[index]);
    } )
    ...
    for( let row=0; row < dataByFeature[0].length; row++) {
      featureNames.forEach( (feature, index) => {
        ...
        expect(data.at(0).children.toString()).toEqual(dataByFeature[index][row].toString());
      })
    }
    ...
    // work to check predDrop.at(0).value with this type of dropdown
    // which is part of a Field.
    //console.log('prepDrop ', predDrop.at(0).value);
    //
  ...
  it('test set-all-features-to menu', () => {
    ...
    expect(menu.at(0).open).toEqual(true);
